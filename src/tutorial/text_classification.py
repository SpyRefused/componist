...
# It has already been preprocessed such that the reviews (sequences of words)
# have been converted to sequences of integers,
...
# for indices 3 and 5, which are ones.
...
#
# - Alternatively, we can pad the arrays so they all have the same length, then create an integer
# tensor of shape num_examples * max_length.
...
#
# Next, a GlobalAveragePooling1D layer returns a fixed-length output vector for each example by averaging over
...
# Using the sigmoid activation function, this value is a float between 0 and 1, representing a probability,
# or confidence level.
...
# However, it makes the network more computationally expensive and may lead to learning unwanted patterns—patterns
# that improve performance on training data but not on the test data.
# This is called overfitting, and we'll explore it later.
#
# 4.2) Loss function and optimizer
# A model needs a loss function and an optimizer for training.
# Since this is a binary classification problem and the model outputs of a probability (a single-unit layer with a
# sigmoid activation), we'll use the binary_crossentropy loss function.
